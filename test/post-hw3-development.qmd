...
1a. How have total agrifood CO2 emissions changed over time in China vs. US

1b. How have agrifood CO2 emissions per capita changed over time in China vs. US

2.  What are the key contributors to US agrifood CO2 emissions in 2020 vs 1990 in China and USA?

3.  How have CO2 emissions in the Food sector changed over time in the US and China?
...
**Explain which variables from your data set(s) you will use to answer your question(s).** After wrangling my agrifood CO2 emissions data and creating new columns and calculating mean values for certain variables, I plan to use:

-   time (year)

-   country (selecting US and China (mainland) after renaming appropriately)

-   total population (column I created from male pop + female pop, used to look at emissions per capita, another column I created),

-   emissions per capita

-   all of the different emissions sources (I end up making an emissions_source column in a pivot_longer step), focusing on highest emission sources and food-specific sources.
...
<https://www.data-to-viz.com/graph/sankey.html> Sankey Diagram (scrapped)

I found [this](https://www.cedricscherer.com/img/gallery-tidytuesday//TidyTuesday_Week_2020_31_Palmer_Penguins.jpg) infographic-style visualization set by Cédric Scherer using the Palmer Penguins dataset from a Tidy Tuesday event:

![](figs/TidyTuesday_Week_2020_31_Palmer_Penguins.png){width="539"}

I like the layout of this visualization, and I hope to eventually incorporate the fun, consistent thematic colors and icons (penguin drawings) in my final infographic. I like the use of color in the plot text elements that reduces the need for a legend. I also like the annotations, but I'm unsure of what kind of information would be relevant for annotations on my plots, so I'd appreciate feedback or suggestions on what I can add in that sense.

![](figs/bigmac_index.png)

The second interesting/relevant(?) infographic that I found was this one on the [Big Mac Index](https://www.christophenicault.com/media/visualizations/bigmac_index.png) by Christophe Nicault. I like the way that all of the countries are situated in context on the world map, but then certain countreis are highlighted and have more information provided about/data visualized on them in pop-out line and dot plots without the overall infographic feeling too cluttered or text-heavy. I am considering showing the country shapes with representative areas (scaled) for the US and China in my final infographic because of this visualization. I think that the text on the line and dot plots gets a little messy, but I like the use of color in the dots on the plots to indicate values being over or under a certain threshold.
...
![](figs/EDS240-drawn-plots-1-2.png){fig-alt="Three hand-drawn plots show three different types of data visualizations for data about agri-food CO2 emissions in the USA and China from 1990-2020. The first plot is comprised of two line plots, one showing total emissions and the other showing emissions per capita. They both show the USA and China from 1990-2020, differentiated by line type and color. The second plot is a dumbbell plot of top agri-food emissions sources in the USA and China, with emissions levels for each source shown in 1990 compared to 2020, differentiating years by color. The third plot shows a scatter plot with linear regression applied to facet-wrapped emissions categories in the Food Sector, with USA and China differentiated by color. Overall, trends show that China has extremely high total emissions, with lower emissions per capita than the USA." fig-align="center" width="534"}

![](figs/EDS240-drawn-plot3.png){fig-alt="This third hand-drawn plot shows facet-wrapped scatter plots with linear regression applied, and the plots are facet-wrapped by emissions categories in the Food Sector, with USA and China differentiated by color. Overall, there appear to be varying degrees of positive, upward trends in this mock-up of the data, with emissions increasing over time in both countries in all food-sector emission sources." fig-align="center" width="585"}

Note: These look remarkably sloppy compared to what they look like in my notebook (they're notably smaller IRL... and I think the way that I scanned them to upload them as PNGs made them look extra-horrific). Yikes! Sorry...
...
theme(
  ...
  # --------------- y-axis ----------------------
  axis.title.y = element_markdown(family = "Futura",
                              vjust = 2,
                              face = "bold",
                              size = 12,
...
plot1 + plot_annotation(
  title = glue::glue("Total Agri-food CO<sub>2</sub> Emissions in USA and China"),
...
theme(
  ...
  # --------------- y-axis ----------------------
  axis.title.y = ggtext::element_markdown(family = "Futura",
                              vjust = 2,
                              face = "bold",
                              size = 12),
  axis.text.y = element_text(family = "Futura"),
...
This plot honestly surprised me, because I expected food transport emissions to go up significantly in both countries as more of the population moved into cities, meaning that more food had to travel farther to get to the higher urban densities of each country's populations. I did also spend a long time trying to get the different elements to be laid out in a more aesthetically pleasing way, including changing the spacing of the panel elements (faceted plots). I've spaced them out in the x-direction, but not y-direction. I've been staring at all of this for so long that I'm not sure if it looks weird to have them not spaced out in the y-direction?? I needed to space them out horizontally to prevent the year (x-axis) labels on the Food Processing plot from overlapping onto the neighboring plots.
...
`savanna_fires`, `forest_fires`, `fire_in_humid_tropical_forests`, `fires_in_organic_soils`

Forests:

`forestland` (sink / -), `net_forest_conversion`
...
`rice_cultivation`, `crop_residues`

`on_farm_energy_use` = `on_farm_electricity_use` + ?

`manure_management` = `manure_applied_to_soils` + `manure_left_on_pasture` ?

Agrichemical:

`pesticides_manufacturing`, `fertilizers_manufacturing`

FOOD:

`food_household_consumption`, `food_packaging`, `food_transport`, `food_retail`, `food_processing`

-   other: `agrifood_systems_waste_disposal` `ippu` (the explanation for this variable in the dataset page on Kaggle / metadata is extremely limited, which is frustrating because this is such a big source)
...
Note: I'm not going to include this one in my final infographic, I just got excited about the idea of it and failed to realize that it was not in fact such a good idea. It is interactive, which is fun but also not allowed in this assignment/project I think?
