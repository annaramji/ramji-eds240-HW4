...
## Plot 2.1: Sankey (not static, so scrapped)
...
## Plot 2.2: Dumbbell Plot

Bar plot of emissions by source in US, China (faceted)
...
usa_china_subset <- usa_china_90_20 |> 
  select(-c(forestland, fires_in_organic_soils,
            ...
            drained_organic_soils_co2, 
            pesticides_manufacturing)) |> 
  pivot_longer(cols = -c(country, year, rural_pop, urban_pop, total_population_male, total_population_female, total_pop, avg_temp_c, emissions_per_capita, total_emissions, ippu),
               names_to = "emission_source",
               values_to = "emissions_kt") |> 
...
geom_segment(aes(x = emissions_1990, 
  ...
  theme_bw() +
  labs(
    title = "Agrifood CO2 Emissions in USA vs. China in 1990 vs. 2020",
    y = "",
    x = "CO2 Emissions (kt)"
  ) +
  theme(
...
food_emissions <- usa_china_pivot |> 
  filter(emission_source %in% c("food_household_consumption", "food_packaging",
                                "food_processing", "food_retail",
                                "food_transport"))
